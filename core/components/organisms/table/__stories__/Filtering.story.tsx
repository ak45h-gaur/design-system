...
export const filtering = () => {
  ...
  const TableHeader: React.FC = memo(() => {
    ...
    const handleRoleSelect: SelectProps['onSelect'] = useCallback(
      (selectedOption: any) => {
        const selectedRoles = Array.isArray(selectedOption) ? selectedOption : [selectedOption];
        setRoleFilter(selectedRoles.map((opt: any) => opt.value || opt));
      },
      [setRoleFilter]
    );
    ...
    const schema = useMemo<GridProps['schema']>(
      () => [
        ...
        },
        {
          ...
          displayName: 'Role',
          sorting: true,
          // TODO: Update filters to match your filter options constant
          filters: ROLE_OPTIONS,
          // TODO: Update onFilterChange to match your data field name
          onFilterChange: (rowData: GridProps['data'][number], filters: any[]) => {
            return filters.some((filter) => rowData.role === filter);
          },
        },
        {
          name: 'status',
          ...
          // TODO: Update onFilterChange to match your data field name
          onFilterChange: (rowData: GridProps['data'][number], filters: any[]) => {
            return filters.some((filter) => rowData.status === filter);
          },
